...
- The default JDK on this machine is 21; the build needs 25. Prefix commands with `JAVA_HOME=$HOME/.sdkman/candidates/java/25.0.3-tem` if `./mvnw` reports `release version 25 not supported`.
...
- **Telemetry config that silently produces an empty Grafana**: `management.tracing.sampling.probability` defaults to `0.1` (set to `1.0` here); timers publish no histogram unless `management.metrics.distribution.percentiles-histogram.*` asks, so every p95 panel would be empty; and `otel-lgtm` keeps Grafana/Loki/Prometheus/Tempo under `/data`, so without the `lgtm-data` volume every restart wipes the lot.
